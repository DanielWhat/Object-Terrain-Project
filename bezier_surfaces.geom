...
void main()
{
    ...
    for (i = 0; i < gl_in.length; i++) {
        ...
        view_vector = normalize(camera_point - gl_in[i].gl_Position.xyz);
        half_way_vector = normalize(view_vector + light_vector);

        l_dot_n = max(dot(normal_vector, light_vector), 0);
        h_dot_n = max(dot(normal_vector, half_way_vector), 0);
        EmitVertex();
    }
    EndPrimitive();
}
